...
Yammer-apns is a gem for accessing the Apple Push Notification Service that allows
...
Paul Gebheim (http://github/justintv/APNS)

h2. Install

<pre>
sudo gem install yammer-apns
</pre>
...
This pem file should be stored somewhere secure that your application can
access. Next, set the yammer-apns configuration parameters:
...
yammer-apns provides a simple api to access Apple's feedback service. Below is an example for setting the feedback time on an ActiveRecord object corresponding to a device token.
